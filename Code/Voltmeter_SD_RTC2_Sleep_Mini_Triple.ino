/*
 * This setup allows you to measure the voltage of three objects (Fuel Cells, Batteries, etc.) up to 3000 mV. 
 * The setup is currently calibrated to measure once every 10 minutes and goes to low power sleep when not actively measuring anything.
...
//Calibration from VCC
//float Cali = 3.36; // USB
float Cali = 3.28; // LiPo
...
//Change the 3.35 to whatever the voltage is coming from the VCC pin onto the breadboard. 
    ...
    input_voltage3 = (analog_value3 * (Cali)*1000) / 1024.0; 
...
//Event 2  
  ...
  if (voltData) {
    ...
    voltData.print(", ");
    voltData.print(input_voltage2);
    voltData.print(", ");
    voltData.println(input_voltage3);
    voltData.close();
...
void Going_To_Sleep(){
    ...
    int NewTime;
    NewTime = minute(tDown) + time_interval;
    if (NewTime >= 60) {
      NewTime = NewTime - 60;
    }
    
    RTC.setAlarm(ALM1_MATCH_MINUTES , 59, NewTime, 0, 0);
